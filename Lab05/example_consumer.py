...
def get_road_only_frame(frame, trapezoid_points):
    ...
    black_frame = gray_frame.copy()
    black_frame.fill(0)

    black_frame_with_trapezoid = cv2.fillConvexPoly(black_frame, trapezoid_points, (255, 255, 255))

    return cv2.bitwise_and(gray_frame, black_frame_with_trapezoid)
...
def get_sobel_frame_binarized():
    ...
    # _, sobel_frame_final = cv2.threshold(sobel_frame_final, 50, 255, cv2.THRESH_BINARY)
...
while True:
    ...
    trapezoid_points = get_trapezoid_array(shrink_frame.shape[1], shrink_frame.shape[0])

    road_only_frame = get_road_only_frame(shrink_frame, trapezoid_points)
    ...
    update_prev_values(left_top_x, left_bottom_x, right_top_x, right_bottom_x, left_top_y, left_bottom_y, right_top_y,
                       right_bottom_y)
